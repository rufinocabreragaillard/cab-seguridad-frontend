...
import { categoriasCaractGeneDocsApi } from '@/lib/api'
import type { CategoriaCaractGeneDocs, TipoCaractGeneDocs } from '@/lib/tipos'
import { exportarExcel } from '@/lib/exportar-excel'

type TabActiva = 'categorias' | 'tipos'

export default function PaginaCategoriasCaracteristicaGeneDocs() {
  const [tabActiva, setTabActiva] = useState<TabActiva>('categorias')

  // ── Categorias ────────────────────────────────────────────────────────────
  const [categorias, setCategorias] = useState<CategoriaCaractGeneDocs[]>([])
  const [cargandoCat, setCargandoCat] = useState(true)
  const [busquedaCat, setBusquedaCat] = useState('')
  const [modalCat, setModalCat] = useState(false)
  const [catEditando, setCatEditando] = useState<CategoriaCaractGeneDocs | null>(null)
  const [formCat, setFormCat] = useState({
    codigo_cat_gene_docs: '', nombre_cat_gene_docs: '', descripcion_cat_gene_docs: '',
    es_unica_gene_docs: false, editable_en_detalle_gene_docs: true,
  })
  const [guardandoCat, setGuardandoCat] = useState(false)
  const [errorCat, setErrorCat] = useState('')
  const [confirmCat, setConfirmCat] = useState<CategoriaCaractGeneDocs | null>(null)
  const [eliminandoCat, setEliminandoCat] = useState(false)

  // ── Categoria seleccionada (para Tipos) ─────────────────────────────────
  const [catSeleccionada, setCatSeleccionada] = useState<CategoriaCaractGeneDocs | null>(null)

  // ── Tipos ─────────────────────────────────────────────────────────────────
  const [tipos, setTipos] = useState<TipoCaractGeneDocs[]>([])
  const [cargandoTipos, setCargandoTipos] = useState(false)
  const [modalTipo, setModalTipo] = useState(false)
  const [tipoEditando, setTipoEditando] = useState<TipoCaractGeneDocs | null>(null)
  const [formTipo, setFormTipo] = useState({ codigo_tipo_gene_docs: '', nombre_tipo_gene_docs: '' })
  const [guardandoTipo, setGuardandoTipo] = useState(false)
  const [errorTipo, setErrorTipo] = useState('')
  const [confirmTipo, setConfirmTipo] = useState<TipoCaractGeneDocs | null>(null)
  ...
  const cargarCategorias = useCallback(async () => {
    setCargandoCat(true)
    try {
      setCategorias(await categoriasCaractGeneDocsApi.listar())
    } finally {
  ...
  const cargarTipos = useCallback(async () => {
    ...
    try {
      setTipos(await categoriasCaractGeneDocsApi.listarTipos(catSeleccionada.codigo_cat_gene_docs))
    } finally {
  ...
  const abrirNuevaCat = () => {
    setCatEditando(null)
    setFormCat({ codigo_cat_gene_docs: '', nombre_cat_gene_docs: '', descripcion_cat_gene_docs: '', es_unica_gene_docs: false, editable_en_detalle_gene_docs: true })
    setErrorCat('')
    setModalCat(true)
  }

  const abrirEditarCat = (c: CategoriaCaractGeneDocs) => {
    setCatEditando(c)
    setFormCat({
      codigo_cat_gene_docs: c.codigo_cat_gene_docs,
      nombre_cat_gene_docs: c.nombre_cat_gene_docs,
      descripcion_cat_gene_docs: c.descripcion_cat_gene_docs || '',
      es_unica_gene_docs: c.es_unica_gene_docs,
      editable_en_detalle_gene_docs: c.editable_en_detalle_gene_docs,
    })
    setErrorCat('')
    setModalCat(true)
  }
  ...
  const guardarCat = async () => {
    if (!formCat.codigo_cat_gene_docs.trim() || !formCat.nombre_cat_gene_docs.trim()) {
    ...
    try {
      if (catEditando) {
        await categoriasCaractGeneDocsApi.actualizar(catEditando.codigo_cat_gene_docs, {
          nombre_cat_gene_docs: formCat.nombre_cat_gene_docs,
          descripcion_cat_gene_docs: formCat.descripcion_cat_gene_docs || undefined,
          es_unica_gene_docs: formCat.es_unica_gene_docs,
          editable_en_detalle_gene_docs: formCat.editable_en_detalle_gene_docs,
        })
      } else {
        await categoriasCaractGeneDocsApi.crear({
          codigo_cat_gene_docs: formCat.codigo_cat_gene_docs.toUpperCase(),
          nombre_cat_gene_docs: formCat.nombre_cat_gene_docs,
          descripcion_cat_gene_docs: formCat.descripcion_cat_gene_docs || undefined,
          es_unica_gene_docs: formCat.es_unica_gene_docs,
          editable_en_detalle_gene_docs: formCat.editable_en_detalle_gene_docs,
  ...
  const eliminarCat = async () => {
    ...
    try {
      await categoriasCaractGeneDocsApi.desactivar(confirmCat.codigo_cat_gene_docs)
      setConfirmCat(null)
      cargarCategorias()
    } finally {
  ...
  const abrirNuevoTipo = () => {
    ...
    setFormTipo({ codigo_tipo_gene_docs: '', nombre_tipo_gene_docs: '' })
    setErrorTipo('')
    setModalTipo(true)
  }

  const abrirEditarTipo = (t: TipoCaractGeneDocs) => {
    setTipoEditando(t)
    setFormTipo({ codigo_tipo_gene_docs: t.codigo_tipo_gene_docs, nombre_tipo_gene_docs: t.nombre_tipo_gene_docs })
    setErrorTipo('')
    setModalTipo(true)
  }
  ...
  const guardarTipo = async () => {
    ...
    if (!formTipo.codigo_tipo_gene_docs.trim() || !formTipo.nombre_tipo_gene_docs.trim()) {
    ...
    try {
      if (tipoEditando) {
        await categoriasCaractGeneDocsApi.actualizarTipo(catSeleccionada.codigo_cat_gene_docs, tipoEditando.codigo_tipo_gene_docs, {
          nombre_tipo_gene_docs: formTipo.nombre_tipo_gene_docs,
        })
      } else {
        await categoriasCaractGeneDocsApi.crearTipo(catSeleccionada.codigo_cat_gene_docs, {
          codigo_cat_gene_docs: catSeleccionada.codigo_cat_gene_docs,
          codigo_tipo_gene_docs: formTipo.codigo_tipo_gene_docs.toUpperCase(),
          nombre_tipo_gene_docs: formTipo.nombre_tipo_gene_docs,
        })
      }
      setModalTipo(false)
  ...
  const eliminarTipo = async () => {
    ...
    try {
      await categoriasCaractGeneDocsApi.desactivarTipo(catSeleccionada.codigo_cat_gene_docs, confirmTipo.codigo_tipo_gene_docs)
      setConfirmTipo(null)
      cargarTipos()
    } finally {
  ...
  const selectorCategoria = (
    <div className="mb-4">
      ...
      <select
        ...
        value={catSeleccionada?.codigo_cat_gene_docs || ''}
        onChange={(e) => {
          const cat = categorias.find((c) => c.codigo_cat_gene_docs === e.target.value) || null
          setCatSeleccionada(cat)
        }}
      >
        <option value="">Seleccione una categoría...</option>
        {categorias.filter((c) => c.activo).map((c) => (
          <option key={c.codigo_cat_gene_docs} value={c.codigo_cat_gene_docs}>{c.nombre_cat_gene_docs}</option>
        ))}
      </select>
    </div>
  )
  ...
  return (
    <div className="flex flex-col gap-6 max-w-6xl">
      <div>
        <h2 className="text-2xl font-bold text-texto">Categorías Genéricas de Documentos</h2>
        <p className="text-sm text-texto-muted mt-1">Categorías y tipos transversales que aplican a todos los grupos</p>
      </div>
            ...
            <div className="flex gap-2 ml-auto">
              <Boton variante="contorno" tamano="sm" disabled={catsFiltradas.length === 0}
                onClick={() => exportarExcel(catsFiltradas as unknown as Record<string, unknown>[], [
                  { titulo: 'Código', campo: 'codigo_cat_gene_docs' },
                  { titulo: 'Nombre', campo: 'nombre_cat_gene_docs' },
                  { titulo: 'Única', campo: 'es_unica_gene_docs', formato: (v: unknown) => (v ? 'Sí' : 'No') },
                  { titulo: 'Editable', campo: 'editable_en_detalle_gene_docs', formato: (v: unknown) => (v ? 'Sí' : 'No') },
                  { titulo: 'Estado', campo: 'activo', formato: (v: unknown) => (v ? 'Activo' : 'Inactivo') },
                ], 'categorias-genericas-docs')}>
            ...
            <TablaCuerpo>
              {cargandoCat ? (
                ...
                <TablaFila key={c.codigo_cat_gene_docs}>
                  ...
                  <TablaTd><Insignia variante={c.editable_en_detalle_gene_docs ? 'exito' : 'neutro'}>{c.editable_en_detalle_gene_docs ? 'Sí' : 'No'}</Insignia></TablaTd>
      ...
      {tabActiva === 'tipos' && (
        <>
          ...
          {catSeleccionada ? (
            <>
              <div className="flex items-center gap-3">
                <span className="text-sm text-texto-muted">Tipos de: <strong>{catSeleccionada.nombre_cat_gene_docs}</strong></span>
              ...
              <Tabla>
                ...
                <TablaCuerpo>
                  {cargandoTipos ? (
                    ...
                    <TablaFila key={t.codigo_tipo_gene_docs}>
                      <TablaTd><code className="text-xs bg-fondo px-2 py-1 rounded font-mono">{t.codigo_tipo_gene_docs}</code></TablaTd>
                      <TablaTd className="font-medium">{t.nombre_tipo_gene_docs}</TablaTd>
      ...
      {/* Modal Categoria */}
      <Modal abierto={modalCat} alCerrar={() => setModalCat(false)} titulo={catEditando ? `Editar: ${catEditando.nombre_cat_gene_docs}` : 'Nueva categoría genérica'}>
        <div className="flex flex-col gap-4">
          <Input etiqueta="Código *" value={formCat.codigo_cat_gene_docs}
            onChange={(e) => setFormCat({ ...formCat, codigo_cat_gene_docs: e.target.value.toUpperCase() })}
            placeholder="Ej: METADATOS" disabled={!!catEditando} />
          <Input etiqueta="Nombre *" value={formCat.nombre_cat_gene_docs}
            onChange={(e) => setFormCat({ ...formCat, nombre_cat_gene_docs: e.target.value })}
          ...
          <div>
            ...
            <textarea className="w-full rounded-lg border border-borde bg-fondo-tarjeta px-3 py-2 text-sm text-texto placeholder:text-texto-muted focus:border-primario focus:ring-1 focus:ring-primario outline-none resize-y min-h-[60px]"
              value={formCat.descripcion_cat_gene_docs}
              onChange={(e) => setFormCat({ ...formCat, descripcion_cat_gene_docs: e.target.value })} />
          </div>
          <div className="flex gap-6">
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={formCat.es_unica_gene_docs}
                onChange={(e) => setFormCat({ ...formCat, es_unica_gene_docs: e.target.checked })}
                className="rounded border-borde" />
              Única por documento
            </label>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" checked={formCat.editable_en_detalle_gene_docs}
                onChange={(e) => setFormCat({ ...formCat, editable_en_detalle_gene_docs: e.target.checked })}
                className="rounded border-borde" />
              Editable en detalle
            </label>
          </div>
      ...
      {/* Modal Tipo */}
      <Modal abierto={modalTipo} alCerrar={() => setModalTipo(false)} titulo={tipoEditando ? `Editar: ${tipoEditando.nombre_tipo_gene_docs}` : 'Nuevo tipo'}>
        <div className="flex flex-col gap-4">
          <Input etiqueta="Código *" value={formTipo.codigo_tipo_gene_docs}
            onChange={(e) => setFormTipo({ ...formTipo, codigo_tipo_gene_docs: e.target.value.toUpperCase() })}
            placeholder="Ej: AUTOR, FECHA_PUB" disabled={!!tipoEditando} />
          <Input etiqueta="Nombre *" value={formTipo.nombre_tipo_gene_docs}
            onChange={(e) => setFormTipo({ ...formTipo, nombre_tipo_gene_docs: e.target.value })}
      ...
      <ModalConfirmar abierto={!!confirmCat} alCerrar={() => setConfirmCat(null)} alConfirmar={eliminarCat}
        titulo="Desactivar categoría" mensaje={confirmCat ? `¿Desactivar "${confirmCat.nombre_cat_gene_docs}"?` : ''} textoConfirmar="Desactivar" cargando={eliminandoCat} />
      <ModalConfirmar abierto={!!confirmTipo} alCerrar={() => setConfirmTipo(null)} alConfirmar={eliminarTipo}
        titulo="Desactivar tipo" mensaje={confirmTipo ? `¿Desactivar "${confirmTipo.nombre_tipo_gene_docs}"?` : ''} textoConfirmar="Desactivar" cargando={eliminandoTipo} />
    </div>
  )
}
